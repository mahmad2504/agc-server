...
class Filter {
	...
	function __construct($name,$query,$rebuild=0,$cached=-1)
	{
		...
		if(strlen($query) == 0)
			return;
		$fields = 'key,status,summary,start,end,timeoriginalestimate,timespent,labels,assignee,created,issuetype,issuelinks,emailAddress,aggregatetimespent,subtasks,story_points';
		$this->query = $query;
