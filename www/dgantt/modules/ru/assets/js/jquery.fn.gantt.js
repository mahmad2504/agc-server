...
(function ($) {
    ...
    $.fn.gantt = function (options) {
		...
		var jira_check = "unchecked"
		if(settings.jira == 1)
			jira_check = "checked";
		
		//console.log(jira_check);
                        ...
                        .append($('<div class="nav-slider" />')
                            ...
                            .append($('<span role="button" class="nav-link nav-next-week"/>')
                                ...
                                //    .click(function () {
                                 //       core.zoomInOut(element, 1);
                                 //   }))
				.append($('<input type="checkbox" id="jira"  '+jira_check+'>&nbspShow Logged Work&nbsp</input>')
                                    .click(function () {
	...
	showHolidays : function (element, val) {
		...
		if(document.getElementById("jira").checked)
			jira = 1;

		settings.jira = jira;
		var url=BuildUrl('ru','data',settings);
		//$.getJSON("timechart?board="+settings.board+"&resource=data&vacations="+vacations+"&scale="+settings.scale+"&oa="+oa, function (jsData) {
		$.getJSON(url, function (jsData) 
		{
			jsData = GetData(jsData);
            element.data = jsData;
            core.init(element);
        });
			},
		...
		var url=BuildUrl('timechart','data',settings);
		//$.getJSON("timechart?board="+settings.board+"&resource=data&vacations="+vacations+"&scale="+settings.scale+"&oa="+oa, function (jsData) {
		$.getJSON(url, function (jsData) 
		{
			jsData = GetData(jsData);
