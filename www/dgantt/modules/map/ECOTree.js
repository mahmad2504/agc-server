...
ECOTree = function (obj, elm) {
	this.config = {
		...
		iRootOrientation : 3,//ECOTree.RO_BOTTOM,
...
ECOTree._canvasNodeOnMouseOverHandler = function (tree,evt,meta,id) {
	//if (target != nodeid) return;
	//tree.selectNode(nodeid,true);
	var d = document.getElementById('popup');
	node = tree.GetNode(id);
	//node = ECOTree.GetNode(id);
	//console.log(node.end);
	if(d.innerHTML != meta)
	{
		d.style.position = "absolute";
		d.style.left = evt.pageX+'px';
		d.style.top = evt.pageY+25+'px';
		d.innerHTML = meta;
		if(node.status == 'RESOLVED')
			d.innerHTML += "<br>Resolved";
		else
		{
			if(node.progress > 0)
				d.innerHTML += "<br>"+"Progress is "+node.progress+"%";
			if(node.deadline != null)
			{
				var deadlinedate = new Date(node.deadline);
				d.innerHTML += "<br>"+"Deadline is "+deadlinedate.toDateString();
			}
			var enddate = new Date(node.end);
			d.innerHTML += "<br>"+"Expected Finish is "+enddate.toDateString();
		}
...
ECOTree._canvasNodeOnMouseOutHandler  = function (tree,target,nodeid) {
	if (target != nodeid) return;
	//tree.selectNode(nodeid,true);
				...
				case "CANVAS":
					...
					switch (this.config.nodeFill) {
						case ECOTree.NF_GRADIENT:							
							...
							var w = node.progress/100*node.w;
							
							this.ctx.fillRect(node.XPosition,node.YPosition,w,node.h);
					...
					if(node.status != 'RESOLVED')
					{
						if(node.delayed == 1)
						{
							...
							ECOTree._roundedRect(this.ctx,node.XPosition-3,node.YPosition-3,node.w+6,node.h+6,5);
						}
						else if(node.deadline != null)
						{
					...
					s.push('onmouseover="javascript:ECOTree._canvasNodeOnMouseOverHandler('+this.obj+',event,\''+node.meta+'\',\''+node.id+'\');" ');										
					s.push('onmouseout="javascript:ECOTree._canvasNodeOnMouseOutHandler('+this.obj+',event.target.id,\''+node.id+'\');" ');										
...
ECOTree.prototype.GetNode = function (id) 
{
	for (var k = 0; k < this.nDatabaseNodes.length; k++)
	{
